...
#ifndef CONFIG_JOYSTICK_DEFPRIO
#  define CONFIG_JOYSTICK_DEFPRIO 80
#endif

#ifndef CONFIG_JOYSTICK_STACKSIZE
#  define CONFIG_JOYSTICK_STACKSIZE 2048 
#endif

#ifndef CONFIG_JOYSTICK_NPOLLWAITERS
#  define CONFIG_JOYSTICK_NPOLLWAITERS 2
#endif


#define DEV_FORMAT          "/dev/js%d"
#define DEV_NAMELEN         9 
...
#define JOY_AXIS_Y		0
#define JOY_AXIS_X		1
#define JOY_DIR_BUTTON		2
#define JOY_AXIS_Z		3
#define JOY_UNK_0		4
#define JOY_THROTTLE		5
#define JOY_UNK_1		6
...
struct usbhost_state_s
{
	...
	char                                 devchar;      /* Character identifying the /dev/js[n] device */
	...
	size_t				     bufdata_len;   /* Size of the buf data */
	usbhost_ep_t                         epin;         /* IN endpoint */
	//AMX usbhost_ep_t                         epout;        /* OUT endpoint */
	pid_t                                pollpid;      /* PID of the poll task */
	size_t                               out_seq_num;  /* The sequence number for outgoing packets */
	struct joystick_buttonstate_s rpt;          /* The latest report out of the controller. */
#ifndef CONFIG_DISABLE_POLL
	struct pollfd 			    *fds[CONFIG_JOYSTICK_NPOLLWAITERS];
...
static int usbhost_joystick_poll(int argc, char *argv[]);
...
static const const struct usbhost_id_s g_joystick_id[] =
{
	/* TODO Speicify joystick */
	/* Logitech 3D */
	{
		/* base -- Must be one of the USB_CLASS_* definitions in usb.h */
		USB_CLASS_HID,

		/* subclass -- depends on the device */
		0x0000,

		/* proto -- depends on the device    */
		0x0000,
...
static struct usbhost_registry_s g_joystick =
{
	NULL,                   /* flink    */
...
static const struct file_operations g_joystick_fops =
{
	usbhost_open,            /* open      */
	usbhost_close,           /* close     */
	usbhost_read,            /* read      */
...
/* This is a bitmap that is used to allocate device names /dev/js-z. */
...
{
	static char test = 0;
	(void)snprintf(devname, DEV_NAMELEN, DEV_FORMAT, test); //priv->devchar);
	test++;
}
...
static void usbhost_notify(FAR struct usbhost_state_s *priv)
{
	...
	/* If there are threads waiting on pusbhost_joystick_polloll() for controller data to become available,
...
#ifndef CONFIG_DISABLE_POLL
	for (i = 0; i < CONFIG_JOYSTICK_NPOLLWAITERS; i++)
	{
	...
	while (!priv->disconnected)
	{
		...
		else
		{
			...
			if (priv->open)
			{
				...
				priv->rpt.joy_y =
					((int8_t*)(priv->tbuffer))[JOY_AXIS_Y];
				priv->rpt.joy_x =
					((int8_t*)(priv->tbuffer))[JOY_AXIS_X];
				priv->rpt.dir_button =
					((uint8_t*)(priv->tbuffer))[JOY_DIR_BUTTON];
				priv->rpt.joy_z =
					((uint8_t*)(priv->tbuffer))[JOY_AXIS_Z];
				priv->rpt.unknown_0 =
					((uint8_t*)(priv->tbuffer))[JOY_UNK_0];
				priv->rpt.throttle =
					((uint8_t*)(priv->tbuffer))[JOY_THROTTLE];
				priv->rpt.unknown_1 =
					((uint8_t*)(priv->tbuffer))[JOY_UNK_1];
...
static int usbhost_sample(FAR struct usbhost_state_s *priv,
		FAR struct joystick_buttonstate_s *sample)
{
	irqstate_t flags;
	int ret = -EAGAIN;
	...
	if (priv->valid)
	{
		...
		memcpy(sample, &priv->rpt, sizeof(struct joystick_buttonstate_s));
		...
		FAR const uint8_t *configdesc, int desclen)
{
	...
	//AMX FAR struct usbhost_epdesc_s epoutdesc;
	int remaining;
	uint8_t found = 0;
	bool done = false;
	int ret;
	...
	//AMX memset(&epoutdesc, 0, sizeof(struct usbhost_epdesc_s));
	...
	{
		uinfo("Not a configurating desc\n");
		return -EINVAL;
	}
			...
			case USB_DESC_TYPE_ENDPOINT:
				{
					...
					if ((epdesc->attr & USB_EP_ATTR_XFERTYPE_MASK) == USB_EP_ATTR_XFER_INT)
					{
						...
						if (USB_ISEPOUT(epdesc->addr))
						{
							...
							// AMX epoutdesc.hport        = hport;
							// AMX epoutdesc.addr         = epdesc->addr & USB_EP_ADDR_NUMBER_MASK;
							// AMX epoutdesc.in           = false;
							// AMX epoutdesc.xfrtype      = USB_EP_ATTR_XFER_INT;
							// AMX epoutdesc.interval     = epdesc->interval;
							// AMX epoutdesc.mxpacketsize = usbhost_getle16(epdesc->mxpacketsize);
							// AMX uerr("Interrupt OUT EP addr:%d mxpacketsize:%d\n",
...
#endif
						{
							...
							priv->bufdata_len = epindesc.mxpacketsize;
...
#endif
		uerr("ERROR: Found IF:%s BIN:%s EPOUT:%s\n",
				(found & USBHOST_IFFOUND) != 0  ? "YES" : "NO", "NONE");
		return -EINVAL;
	}
...
static inline int usbhost_devinit(FAR struct usbhost_state_s *priv)
{
	 ...
	 * memory resources, primarily for the dedicated stack (CONFIG_JOYSTICK_STACKSIZE).
	 */
	...
	priv->pollpid = kthread_create("joystick", CONFIG_JOYSTICK_DEFPRIO,
			CONFIG_JOYSTICK_STACKSIZE,
			(main_t)usbhost_joystick_poll,
			(FAR char * const *)NULL);
	if (priv->pollpid < 0)
	{
	...
	usbhost_mkdevname(priv, devname);
	ret = register_driver(devname, &g_joystick_fops, 0666, priv);
	...
	if (ret != OK) {
		uerr("Failed allocating block driver\n");
	}
errout:
	usbhost_takesem(&priv->exclsem);
	priv->crefs--;
...
static ssize_t usbhost_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	memcpy(buffer, &sample, sizeof(struct joystick_buttonstate_s));

	ret = sizeof(struct joystick_buttonstate_s);
...
errout:
	ret = -EINVAL;
	iinfo("No internale information: %d\n", ret);
	return ret;
}
		...
		bool setup)
{
	...
	else if (setup)
	{
		...
		if (i >= CONFIG_JOYSTICK_NPOLLWAITERS)
		{
			fds->priv    = NULL;
			ret          = -EBUSY;
			goto errout;
...
int usbhost_joystick_init(void)
{
	...
	return usbhost_registerclass(&g_joystick);
}
